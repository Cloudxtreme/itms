...
function postViewLoginForm() {
	...
	pass = hex_md5(pass);
	$('#vlfResult').attr('src','./?r=host/genpass&id='+id+'&pass='+pass);
...
'columns'=>array(
		...
		array('name'=>'type','value'=>'Lookup::item("HostType",$data->type)','filter'=>Lookup::items('HostType')),
		'ip',
		'location',
//		'login_user',
		array('header'=>'登录信息','type'=>'raw','value'=>'"<a href=\"javascript:openViewLoginForm({$data->id},\'{$data->ip}\')\">查看</a>"', 'filter'=>false),
		array('header'=>'配置','value'=>'"{$data->cores}核,{$data->memory}M内存,{$data->disk}G系统盘,{$data->data}G数据盘". Lookup::item("HostOS",$data->os)." ".$data->osver','filter'=>false),	
		array('header'=>'带宽','value'=>'Lookup::item("HostBandwidthType",$data->bandwidth_type)." {$data->bandwidth}M"', 'filter'=>false),
...
//			'editable'=>array('type'=>'text','title'=>'输入过期日期','url'=>'./?r=host/sinexpire')
			'editable'=>array('type'=>'date','title'=>'输入过期日期','url' => './?r=host/sinexpire', 'options'=>array('datepicker'=>array('language'=>'zh-CN','format'=>'yyyy-mm-dd'))  )
		),
